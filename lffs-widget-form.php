...
$_SESSION['lffs-widget-rand'] = isset($_SESSION['lffs-widget-rand']) ? $_SESSION['lffs-widget-rand'] : rand(100, 999);

// the shortcode
function lffs_widget_shortcode($lffs_atts) {
	...
	// processing form
	if (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['lffs_widget_send']) && isset( $_POST['lffs_widget_nonce'] ) && wp_verify_nonce( $_POST['lffs_widget_nonce'], 'lffs_widget_nonce_action' ) ) {
		// sanitize content
		$post_data = array(
		...
		if ( $value != $_SESSION['lffs-widget-rand'] ) { 
	...
	// set nonce field
	$nonce = wp_nonce_field( 'lffs_widget_nonce_action', 'lffs_widget_nonce', true, false ); 
	...
	$email_form = '<form class="lffs" id="lffs" method="post" '.$action.'>
		...
		<div class="form-group">
			<label for="lffs_captcha">'.sprintf(esc_attr($captcha_label), $_SESSION['lffs-widget-rand']).': <span class="'.(isset($error_class['form_captcha']) ? "error" : "hide").'" >'.esc_attr($error_captcha_label).'</span></label>
		...
		<div class="form-group">
			<button type="submit" name="lffs_widget_send" id="lffs_widget_send" class="btn btn-primary">'.esc_attr($submit_label).'</button>
		</div>
	</form>';
	
	// after form validation
	if ($sent == true) {
		unset($_SESSION['lffs-widget-rand']);
...
add_shortcode('contact-widget', 'lffs_widget_shortcode');
